    ...
    switch ($method) {
        case 'GET':
            if (in_array('Leer', $permisos)) {
                $query = "SELECT idStatus, nombre FROM status;";
        ...
        case 'POST':
            if (in_array('Escribir', $permisos)) {
                ...
                $query = "INSERT INTO status (nombre, fechaCreacion, usuarioCreacion) VALUES (?, ?, ?)";
        
                if ($insert_query = $mysqli->prepare($query)) {
                    $insert_query->bind_param('sss', $nombre, $fechaCreacion, $usuarioCreacion);
                    if ($insert_query->execute()) {
                        echo json_encode(['success' => 'Status  creado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo crear  el Status.']);
                    }
                    $insert_query->close();
                } else {
        ...
        case 'PUT':
            if (in_array('Escribir', $permisos)) {
                ...
                if (!isset($data['idStatus'], $data['nombre'])) {
                    echo json_encode(['error' => 'Datos incompletos.']);
                    exit();
                }
                ...
                $query = "UPDATE status SET nombre = ?, fechaModificacion = ?, usuarioModificacion = ? WHERE idStatus = ?";

                if ($update_query = $mysqli->prepare($query)) {
                    $update_query->bind_param('sssi', $nombre, $fechaModificacion, $usuarioModificacion, $idStatus);
                    if ($update_query->execute()) {
                        echo json_encode(['success' => 'Status actualizado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo actualizar el Status.']);
                    }
                    $update_query->close();
                } else {
                    echo json_encode(['error' => 'No se pudo preparar la consulta.']);
                ...
                if ($delete_query = $mysqli->prepare($query)) {
                    ...
                    if ($delete_query->execute()) {
                        if ($delete_query->affected_rows > 0) {
                            echo json_encode(['success' => 'Status eliminado.']);
                        } else {
                            echo json_encode(['error' => 'Status no encontrado.']);
                        }
                    } else {
