...
try {
    ...
    $permisos = obtenerPermisos($userId, 'Sensor_Tipo', $mysqli);
    ...
    switch ($method) {
        case 'GET':
            if (in_array('Leer', $permisos)) {
                $query = "SELECT idTipoSensor, tipo FROM tipoSensor;";
        ...
        case 'POST':
            if (in_array('Escribir', $permisos)) {
                ...
                $query = "INSERT INTO tipoSensor (tipo, fechaCreacion, usuarioCreacion) VALUES (?, ?, ?)";
        
                if ($insert_query = $mysqli->prepare($query)) {
                    $insert_query->bind_param('sss', $tipo, $fechaCreacion, $usuarioCreacion);
                    if ($insert_query->execute()) {
                        echo json_encode(['success' => 'Tipo de Sensor  creado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo crear  el Tipo de Sensor.']);
                    }
                    $insert_query->close();
                } else {
        ...
        case 'PUT':
            if (in_array('Escribir', $permisos)) {
                ...
                if (!isset($data['idTipoSensor'], $data['tipo'])) {
                    echo json_encode(['error' => 'Datos incompletos.']);
                    exit();
                }
                ...
                $query = "UPDATE tipoSensor SET tipo = ?, fechaModificacion = ?, usuarioModificacion = ? WHERE idTipoSensor = ?";

                if ($update_query = $mysqli->prepare($query)) {
                    $update_query->bind_param('sssi', $tipo, $fechaModificacion, $usuarioModificacion, $idTipoSensor);
                    if ($update_query->execute()) {
                        echo json_encode(['success' => 'Tipo de sensor actualizado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo actualizar el Tipo de Sensor.']);
                    }
                    $update_query->close();
                } else {
                    echo json_encode(['error' => 'No se pudo preparar la consulta.']);
        ...
        case 'DELETE':
            if (in_array('Borrar', $permisos)) {
                ...
                $query = "DELETE FROM tipoSensor WHERE idTipoSensor = ?";
                ...
                if ($delete_query = $mysqli->prepare($query)) {
                    ...
                    if ($delete_query->execute()) {
                        if ($delete_query->affected_rows > 0) {
                            echo json_encode(['success' => 'Tipo de Sensor eliminado.']);
                        } else {
                            echo json_encode(['error' => 'Tipo de Sensor no encontrado.']);
                        }
                    } else {
