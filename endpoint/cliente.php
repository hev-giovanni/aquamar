...
try {
    ...
    $permisos = obtenerPermisos($userId, 'Clientes', $mysqli);
    ...
    switch ($method) {
        case 'GET':
            if (in_array('Leer', $permisos)) {
                $query = "SELECT idCliente, nombre, nit, telefono, correo, direccion, fechaCreacion FROM cliente;";
            ...
            case 'POST':
                if (in_array('Escribir', $permisos)) {
                    ...
                    $query = "INSERT INTO cliente (nombre, nit, telefono, correo, direccion, fechaCreacion, usuarioCreacion) VALUES (?, ?, ?, ?, ?, ?, ?)";
            
                    if ($insert_query = $mysqli->prepare($query)) {
                        // Asegúrate de que el tipo de usuarioCreacion sea correcto
                        $insert_query->bind_param('ssssssi', $nombre, $nit, $telefono, $correo, $direccion, $fechaCreacion, $usuarioCreacion);
        ...
        case 'PUT':
            if (in_array('Escribir', $permisos)) {
                ...
                $query = "UPDATE cliente SET nombre = ?, nit = ?, telefono = ?, correo = ?, direccion = ?, fechaModificacion = ?, usuarioModificacion = ? WHERE idCliente = ?";

                if ($update_query = $mysqli->prepare($query)) {
                    $update_query->bind_param('sssssssi', $nombre, $nit, $telefono, $correo, $direccion, $fechaModificacion, $usuarioModificacion, $idCliente);
                    if ($update_query->execute()) {
                        echo json_encode(['success' => 'Cliente actualizado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo actualizar Cliente.']);
                    }
                    $update_query->close();
                } else {
                    echo json_encode(['error' => 'No se pudo preparar la consulta.']);
