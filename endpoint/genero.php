...
try {
    ...
    $permisos = obtenerPermisos($userId, 'Genero', $mysqli);
    ...
    switch ($method) {
        ...
        case 'POST':
            if (in_array('Escribir', $permisos)) {
                ...
                $query = "INSERT INTO genero (nombre, fechaCreacion, usuarioCreacion) VALUES (?, ?, ?)";
        
                if ($insert_query = $mysqli->prepare($query)) {
                    $insert_query->bind_param('sss', $nombre, $fechaCreacion, $usuarioCreacion);
                    if ($insert_query->execute()) {
                        echo json_encode(['success' => 'Genero  creado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo crear  el Genero.']);
                    }
                    $insert_query->close();
                } else {
        ...
        case 'PUT':
            if (in_array('Escribir', $permisos)) {
                ...
                if (!isset($data['idGenero'], $data['nombre'])) {
                    echo json_encode(['error' => 'Datos incompletos.']);
                    exit();
                }
                ...
                $query = "UPDATE genero SET nombre = ?, fechaModificacion = ?, usuarioModificacion = ? WHERE idGenero = ?";

                if ($update_query = $mysqli->prepare($query)) {
                    $update_query->bind_param('sssi', $nombre, $fechaModificacion, $usuarioModificacion, $idGenero);
                    if ($update_query->execute()) {
                        echo json_encode(['success' => 'Genero actualizado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo actualizar el Genero.']);
                    }
                    $update_query->close();
                } else {
                    echo json_encode(['error' => 'No se pudo preparar la consulta.']);
        ...
        case 'DELETE':
            if (in_array('Borrar', $permisos)) {
                ...
                $query = "DELETE FROM genero WHERE idGenero = ?";
