    ...
    switch ($method) {
        case 'GET':
            if (in_array('Leer', $permisos)) {
                $query = "SELECT idDispositivo, codigoDispositivo, descripcion FROM dispositivo;";
        ...
        case 'POST':
            if (in_array('Escribir', $permisos)) {
                ...
                if (!isset($data['codigoDispositivo'],$data['descripcion'])) {
                ...
                $query = "INSERT INTO dispositivo (codigoDispositivo, descripcion, fechaCreacion, usuarioCreacion) VALUES (?, ?, ?, ?)";
        
                if ($insert_query = $mysqli->prepare($query)) {
                    $insert_query->bind_param('ssss',  $codigoDispositivo, $descripcion, $fechaCreacion, $usuarioCreacion);
                    if ($insert_query->execute()) {
                        echo json_encode(['success' => 'Dispositivo creado.']);
                    } else {
                        echo json_encode(['error' => 'No se pudo crear el Dispositivo.']);
                    }
                    $insert_query->close();
                } else {
            ...
            case 'PUT':
                if (in_array('Escribir', $permisos)) {
                    ...
                    if (!isset($data['idDispositivo'], $data['codigoDispositivo'], $data['descripcion'])) {
                        echo json_encode(['error' => 'Datos incompletos.']);
                        exit();
                    }
                    ...
                    $query = "UPDATE dispositivo SET codigoDispositivo = ?, descripcion = ?, fechaModificacion = ?, usuarioModificacion = ? WHERE idDispositivo = ?";
    
                    if ($update_query = $mysqli->prepare($query)) {
                        $update_query->bind_param('ssssi', $codigoDispositivo, $descripcion, $fechaModificacion, $usuarioModificacion, $idDispositivo);
                        if ($update_query->execute()) {
                            echo json_encode(['success' => 'Dispositivo actualizado.']);
                        } else {
                            echo json_encode(['error' => 'No se pudo actualizar el Dispositivo.']);
                        }
                        $update_query->close();
                    } else {
                        echo json_encode(['error' => 'No se pudo preparar la consulta.']);
        ...
        case 'DELETE':
            if (in_array('Borrar', $permisos)) {
                ...
                $query = "DELETE FROM dispositivo WHERE idDispositivo = ?";
                ...
                if ($delete_query = $mysqli->prepare($query)) {
                    ...
                    if ($delete_query->execute()) {
                        if ($delete_query->affected_rows > 0) {
                            echo json_encode(['success' => 'Dispositivo eliminado.']);
                        } else {
